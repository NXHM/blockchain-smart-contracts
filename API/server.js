...
// Variables globales de Polkadot
let api;
let contract;
let alice;
let aliceNonce; // Nonce global para la cuenta de Alice

// Función de inicialización del API Polkadot y suscripción a eventos
async function init() {
    ...
    // Configuramos la cuenta de Alice (usada para firmar transacciones en Polkadot)
    const keyring = new Keyring({ type: 'sr25519' });
    alice = keyring.addFromUri('//Alice');
    aliceNonce = await api.rpc.system.accountNextIndex(alice.address);

    // Suscribirse a los eventos de la cadena
    api.query.system.events((events) => {
        ...
        events.forEach((record) => {
            try {
                ...
                // Manejar específicamente eventos del smart contract
...
// Función para obtener e incrementar el nonce local
...
// Función para sincronizar el nonce en caso de error
...
let tip = pendingTransactions; // El tip comienza igual al total de transacciones

// Función para crear una nueva cuenta en Polkadot
...
// Función para transferir fondos (usando transferKeepAlive)
async function transferFunds(sender, recipient, amount) {
    const tip = getTip();
    let nonce = getAndIncrementNonce();
...
// Función addUser para ejecutar la transacción en Polkadot y registrar gas consumido
            ...
            addUserTx.signAndSend(alice, { nonce: currentNonce, tip }, ({ events = [], status }) => {
                if (status.isInBlock || status.isFinalized) {
                    ...
                    if (!hasError) {
                        ...
                        resolve({
                            blockHash: status.isFinalized ? status.asFinalized.toString() : 'N/A',
                            gasUsed: res.locals.refTime || 'N/A',
                            proofSizeUsed: res.locals.proofSize || 'N/A',
                            tip: tip.toString()
                        });
                    }
                } else if (status.isError) {
                ...
                if (error.message.includes('1010') || error.message.includes('Invalid Transaction')) {
                    ...
                    res.locals.transactionSuccess = false;
                    res.locals.refTime = 0;
                    res.locals.proofSize = 0;
                    res.locals.tip = '0';
                    res.locals.transactionCount = 0;
                    reject(error);
                }
            });
        }
...
// Estos endpoints permiten crear usuarios de forma escalable en Polkadot sin llamar a Fabric.

app.post('/create_user', async (req, res) => {
    const { name, lastname, dni, email, role } = req.body;
    const newAccount = createNewAccount();
    const userInfo = { name, lastname, dni, email };
    ...
    try {
        ...
        // Llamar la función addUser en el contrato (solo en Polkadot)
...
// (2) OPERACIONES SOBRE EL HME – AQUÍ SE INVOCA FABRIC
// Antes de ejecutar operaciones (leer, actualizar, compartir, etc.) se llama al servidor de Fabric.

app.post('/verify_hyperledger_user', async (req, res) => {
    const { org, dni, role } = req.body;
    if (!dni || !role) {
        return res.status(400).send("dni y role son requeridos");
    }

    // Uso de template string correcto para construir la URL
    try {
        const response = await axios.post(`${FABRIC_SERVER_URL}/verify_user`, {
            org,
            dni,
            role,
        });
...
app.post('/read_hme', async (req, res) => {
    ...
    console.log('Parámetros recibidos en /read_hme:', req.body);
    ...
    try {
        console.log(`Realizando solicitud a Fabric para org: ${org}, readerDni: ${readerDni}, patientDni: ${patientDni}`);
        const response = await axios.post(`http://localhost:4000/read_hme`, {
            org,
            readerDni,
            patientDni,
        });
        console.log('Respuesta de Fabric:', response.data);
        res.status(200).json({
        ...
        console.error('Error en la solicitud a Fabric:', error.message);
        if (error.response) {
            console.error('Código de estado:', error.response.status);
            console.error('Datos del error:', error.response.data);
        }
        res.status(500).json({ error: `Error leyendo HME desde Hyperledger Fabric: ${error.message}` });
    }
});


// Los endpoints para actualizar, compartir, revocar, agregar hash, acuñar NFT, etc. se mantienen llamando a Fabric
// tal como se muestra en los ejemplos siguientes (no se repite todo el código para mantener claridad).

// ...

// Endpoint para obtener el rol de un usuario en Polkadot
app.get('/role/:publicAddress', async (req, res) => {
    ...
    res.locals.testType = testType || 'N/A';
